...
function Likes() {
  ...
  function addToLikes(book) {
    // Aynı ID'ye sahip bir kartın olup olmadığını kontrol et
    const exists = likedBooks.some(item => item.id === book.id);
    if (!exists) {
      // Yeni kartı likedBooks dizisine ekle
      const updatedLikes = [...likedBooks, book];
            ...
            {likedBooks.map((like) => (
              <Card
                ...
                actions={[
                  ...
                  <EllipsisOutlined key="ellipsis" onClick = {() => RemoveCard() } />,
                ]}
              >
                <Meta
                  title={<p>{like.adi}</p>}
